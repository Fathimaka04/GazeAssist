...
UPDATED (per-person EAR calibration fix): CalibrationEngine now runs
TWO phases - open-eye baseline, then a deliberate closed-eye hold -
instead of guessing the closed-eye value from open-eye statistics
alone. process_frame()'s calibration branch now reports which phase
is active via status "calibrating_blink_open" / "calibrating_blink_closed"
instead of a single "calibrating_blink" status.
...
Usage:
    ...
    for frame in cam.frames():
        ...
        if result.status == "calibrating_blink_open":
            ...show result.progress_current/progress_total...
        elif result.status == "calibrating_blink_closed":
            ...prompt user to close eyes and hold...
...
class EngineResult:
    ...
    # calibrating_blink_open / calibrating_blink_closed
...
class GazeBlinkEngine:
    ...
    blink duration classification (short/long) and the updated
    open+closed EAR calibration.
    """
    ...
    def process_frame(self, frame) -> EngineResult:
        ...
        # --- Phase 1: EAR baseline calibration (open-eye, then closed-eye) ---
    ...
    def _finalize_calibration(self):
        ...
        # NEW: print the full EAR calibration summary (open/closed
        # baselines + final threshold) so we can verify the per-person
        # calibration actually produced a sane gap between open and
        # closed EAR, instead of guessing from the old formula.
        print("EAR CALIBRATION SUMMARY:", self.calibration.get_summary())
